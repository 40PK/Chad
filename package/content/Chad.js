...
let analytics = ua("UA-81643761-1", {
  https: true
});
...
class Chad extends React.Component {
  ...
  sendPost(data) {
    ...
    if (listToSend.length < 1) return alert("Select channel(s) to send post");
    if (this.token === null) return alert("Add Admin bot");
    if (data.text.length < 1) return alert("Post text is empty");
      ...
      API.sendMessage(params).then((success => {
        ...
        if (success.ok) {
          post.chats.push({
            ...
            chat_id: success.result.chat.type === "channel" ? success.result.chat.username : success.result.chat.id
          });
          list.splice(0, 1);
          if (list.length) {
            sendRec(list, onend);
  ...
  changePost(data) {
    ...
    function sendRec(list, onend) {
      ...
      if (typeof params.chat_id === "string") params.chat_id = "@" + params.chat_id;
  ...
  newChannel(data) {
    if (!data.name || data.name.length === 0) return alert("Channel name is too short");
    if (!data.username || !usernameRegex.test(data.username)) return alert("Invalid channel username");
  ...
  setAdminBot(token, onPass) {
    if (!token || token.length < 30) return alert("Invalid token");
    if (!tokenRegex.test(token)) return alert("Invalid token format");
    if (onPass) onPass();
  ...
  render() {
    ...
    return React.createElement(MuiThemeProvider, {
      ...
      type: "row"
    }, React.createElement(Fixed, null, React.createElement(Paper, {
      style: paperStyle,
      zDepth: 1,
      rounded: false
      ...
      data: SideBarData,
      local: this.local
    }))), React.createElement(Flex, null, React.createElement(ContentBar, {
      data: ContentBarData,
      local: this.local,
      signal: this.signal
    }))))));
